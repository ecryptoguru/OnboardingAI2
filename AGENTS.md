...
- Next.js 16 notes: the edge middleware (`middleware.ts` / `proxy.ts`) was removed entirely; dashboard auth protection is now client-side via `components/AuthGuard.tsx`. The production build passes `--webpack` (`npm run build`); `next dev` runs on Turbopack (Next 16 default) — the webpack `resolve.fallback` in `next.config.ts` only applies to the build. The obsolete `eslint` config property was removed from `next.config.ts`. `ConvexClientProvider.tsx` fails fast when `NEXT_PUBLIC_CONVEX_URL` is not set (no silent production fallback). Deployment: Vercel only (`vercel.json` — pins `@vercel/next` builder + CSP with `wss://*.convex.cloud`); live at `https://onboardingai2.vercel.app`. Netlify retired 2026-08-16.
