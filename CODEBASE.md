...
6. **Environment Variables:** The following environment variables are used by the app and backend: `NEXT_PUBLIC_CONVEX_URL`, `SITE_URL` (auth library base URL), `CONVEX_DEPLOYMENT`, `SETTINGS_OBFUSCATION_SECRET`, `GOOGLE_CALENDAR_WEBHOOK_TOKEN`, `ZEPTOMAIL_WEBHOOK_SECRET`, `EMAIL_WEBHOOK_SECRET`, `DISABLE_TEST_ENDPOINTS`, `TEST_WEBHOOK_SECRET`, `LLM_DAILY_BUDGET_USD`, `ADMIN_EMAILS`, `SKIP_RATE_LIMITS`, `SENTRY_DSN`, and `NEXT_PUBLIC_SENTRY_DSN`. (`NEXT_PUBLIC_CONVEX_SITE_URL` is docs-reference only — webhooks run on the deployment's built-in `*.convex.site` URL.) All API service keys (Gemini, Serper, Firecrawl, ZeptoMail, Google Calendar) belong in the DB via the Settings page.
