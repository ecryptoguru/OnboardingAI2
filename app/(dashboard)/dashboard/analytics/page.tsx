...
export default function AnalyticsPage() {
  ...
  const classificationCounts = (replyStats ?? []).reduce<Record<string, number>>((acc, r) => {
  ...
  return (
    <div className="p-8 space-y-10">
      ...
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1.5 font-medium">End-to-end outreach success metrics</p>
      </div>

      {/* Funnel */}
      <section>
        <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-widest mb-4">Outreach Funnel</h2>
        {!funnel ? (
                ...
                <div key={step.key} className="flex items-center gap-4">
                  ...
                  <div className="flex-1 bg-card border border-card-border/50 rounded-xl overflow-hidden h-10 relative">
                    <div
                      ...
                      style={{ width: `${pct}%`, minWidth: val > 0 ? "2px" : "0" }}
                    />
                  ...
                  <div className="w-20 shrink-0 text-right">
                    {conv !== null ? (
                      <span className={`text-xs font-bold ${conv >= 30 ? "text-emerald-400" : conv >= 10 ? "text-amber-400" : "text-red-400"}`}>
                        {conv}% ↑
                      </span>
                    ) : (
      ...
      {/* KPI Cards */}
      <section>
        <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-widest mb-4">Key Metrics</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            {
              label: "Reply Rate",
              value: funnel && funnel.outreachActive > 0
            ...
            {
              label: "Meeting Rate",
              value: funnel && funnel.replied + funnel.meetingBooked > 0
            ...
            <div key={card.label} className="bg-card border border-card-border/60 rounded-xl p-5 shadow-sm">
              <p className="text-muted-foreground text-xs font-medium uppercase tracking-wider mb-1">{card.label}</p>
              <p className={`text-3xl font-heading font-bold ${card.color} mb-1`}>{card.value}</p>
      ...
      <section>
        <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-widest mb-4">Email Performance by Step</h2>
        {!emailStats ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-card-border/40">
                {EMAIL_STEPS.map((step) => {
                  ...
                  const openRate = s && s.sent > 0 ? Math.round((s.opened / s.sent) * 100) : 0;
                  return (
                    <tr key={step} className="hover:bg-muted/20 transition-colors">
                      <td className="px-6 py-4 font-medium text-foreground">
                        <span className="inline-flex items-center gap-2">
                          ...
                          {step === 1 ? "Initial Outreach" : step === 2 ? "Follow-up 1" : step === 3 ? "Value Add" : "Break-up"}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right text-foreground font-mono font-medium">{s?.sent ?? 0}</td>
                      <td className="px-6 py-4 text-right text-foreground font-mono">{s?.opened ?? 0}</td>
                      <td className="px-6 py-4 text-right">
                        <span className={`font-bold ${openRate >= 30 ? "text-emerald-400" : openRate >= 15 ? "text-amber-400" : "text-muted-foreground"}`}>
                          {s && s.sent > 0 ? `${openRate}%` : "—"}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-right text-foreground font-mono">{s?.clicked ?? 0}</td>
                      <td className="px-6 py-4 text-right text-red-400 font-mono">{s?.bounced ?? 0}</td>
                    </tr>
                  );
                })}
              </tbody>
      ...
      {totalReplies > 0 && (
        <section>
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-widest mb-4">Reply Intent Breakdown</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {Object.entries(classificationCounts).sort((a, b) => b[1] - a[1]).map(([cls, count]) => {
              ...
              const colorMap: Record<string, string> = {
                meeting_request: "bg-emerald-500/10 text-emerald-400 border-emerald-500/20",
                positive_interest: "bg-blue-500/10 text-blue-400 border-blue-500/20",
                request_info: "bg-sky-500/10 text-sky-400 border-sky-500/20",
                not_interested: "bg-red-500/10 text-red-400 border-red-500/20",
                opt_out: "bg-orange-500/10 text-orange-400 border-orange-500/20",
                out_of_office: "bg-zinc-500/10 text-muted-foreground border-zinc-500/20",
                other: "bg-muted text-muted-foreground border-card-border",
              };
              return (
                <div key={cls} className={`rounded-xl border px-4 py-3 ${colorMap[cls] ?? colorMap.other}`}>
                  <p className="text-xs font-bold uppercase tracking-widest mb-1">{cls.replace(/_/g, " ")}</p>
                  <p className="text-2xl font-heading font-bold">{count} <span className="text-sm font-normal opacity-70">({pct}%)</span></p>
                </div>
              );
            })}
          </div>
