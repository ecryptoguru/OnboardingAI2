...
export default function EnrichmentPage() {
  ...
  const renderUniversityCard = (uni: any, isNew: boolean, isEnriching: boolean) => (
    <div 
      ...
      className="p-4 bg-zinc-900 border border-zinc-800 rounded-lg hover:bg-zinc-800/60 cursor-pointer transition-colors group mb-3"
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="text-sm font-medium text-white truncate">{uni.university_name}</h3>
        ...
        {isEnriching && (
          <div className="flex-shrink-0 flex items-center justify-center text-indigo-400">
        ...
        {!isNew && !isEnriching && uni.lead_tier && (
          <div className="flex-shrink-0">
            <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${
              uni.lead_tier === 'High' ? 'bg-green-500/10 text-green-400 border border-green-500/20' :
  ...
  return (
    <div className="p-8 h-screen max-h-screen overflow-hidden flex flex-col relative text-zinc-200">
      <div className="flex items-start justify-between mb-8 flex-shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-white">Deep Enrichment Engine</h1>
        ...
        {selectedNewIds.size > 0 && (
          <button
            ...
            className="bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium px-4 py-2 rounded-md shadow-[0_0_15px_rgba(79,70,229,0.3)] transition-all flex items-center gap-2"
          >
      ...
      <div className="flex-1 min-h-0 flex gap-6 overflow-hidden">
        ...
        <div className="flex flex-col w-1/3 bg-zinc-950/50 border border-zinc-800/80 rounded-xl overflow-hidden shadow-sm">
          <div className="p-4 border-b border-zinc-800/80 bg-zinc-900/40 flex items-center justify-between">
            <h2 className="text-sm font-semibold text-white flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-zinc-500"></div>
              Pending / New
              <span className="text-xs bg-zinc-800 text-zinc-400 px-2 rounded-full font-medium ml-1">
                {newUniversities ? newUniversities.filter(u => !enrichingIds.has(u._id)).length : 0}
              </span>
            </h2>
            {newUniversities && newUniversities.length > 0 && (
               <button onClick={selectAll} className="text-xs text-indigo-400 hover:text-indigo-300 font-medium">
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
            {newUniversities === undefined ? (
               ...
               Array(4).fill(0).map((_, i) => (
                 <div key={i} className="h-20 bg-zinc-900/50 border border-zinc-800/50 animate-pulse rounded-lg mb-3" />
               ))
