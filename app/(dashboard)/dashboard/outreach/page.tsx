...
export default function OutreachPage() {
  ...
  const repliedCount = (grouped["replied"]?.length ?? 0) + (grouped["meeting_booked"]?.length ?? 0);
  const replyRate = activeCount > 0 ? Math.round((repliedCount / activeCount) * 100) : 0;
    ...
    <div className="p-8 pb-20">
      ...
      <div className="grid grid-cols-5 gap-4 mb-8">
        {[
          ...
          { label: "Replied", value: grouped["replied"]?.length ?? 0, color: "text-emerald-400" },
          { label: "Meetings Booked", value: grouped["meeting_booked"]?.length ?? 0, color: "text-amber-400" },
          { label: "Reply Rate", value: `${replyRate}%`, color: "text-sky-400" },
          { label: "Total Replies (Log)", value: replyCount, color: "text-white" },
        ].map((stat) => (
            ...
            <div key={col.id} className="flex-shrink-0 w-64">
              <div className="flex items-center justify-between mb-3 px-1">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-xs font-bold text-foreground uppercase tracking-widest">{col.label}</h2>
                </div>
              ...
              <div className="flex flex-col gap-2.5 min-h-[100px] p-2 rounded-2xl bg-background border border-card-border/60">
                ...
                {(!grouped[col.id] || grouped[col.id].length === 0) && (
                  <div className="py-10 text-center">
                    <p className="text-zinc-600 text-[10px] font-medium uppercase tracking-tighter italic">Empty</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="w-72 flex-shrink-0">
          <div className="sticky top-0">
            <div className="flex items-center gap-2 mb-3 px-1">
              ...
              <h2 className="text-xs font-bold text-foreground uppercase tracking-widest">Inbound Replies</h2>
              {replyCount > 0 && (
                <span className="bg-blue-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{replyCount}</span>
              )}
            </div>
      ...
      {/* Modals */}
      {showSkipModal && <SkipUniversityModal onClose={() => setShowSkipModal(false)} />}
      {timelineUniId && (
        <>
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-40" onClick={() => setTimelineUniId(null)} />
          <UniversityTimelineDrawer universityId={timelineUniId} onClose={() => setTimelineUniId(null)} />
        </>
      )}
      {simulateUni && (
        <>
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-40" onClick={() => setSimulateUni(null)} />
          <SimulateReplyModal university={simulateUni} onClose={() => setSimulateUni(null)} />
        </>
      )}
...
function KanbanCard({
  ...
  const sequences = useQuery(api.sequences.listByUniversity, { university_id: university._id });

  const activeSeq = sequences?.find(s => s.status === "active" || s.status === "pending_approval");
    ...
    <div className="group relative p-3.5 bg-muted border border-card-border/80 rounded-xl hover:border-zinc-600 hover:-translate-y-0.5 transition-all duration-200 cursor-pointer shadow-sm overflow-hidden">
      ...
      <div className="flex justify-between items-start mb-2.5">
        ...
        {university.outreach_stage === "meeting_booked" && (
          <span className="text-[9px] font-bold text-amber-400">📅 Meeting</span>
        )}
      </div>
      ...
      {activeSeq && (
        <div className="mb-2.5">
          <div className="flex justify-between text-[9px] text-muted-foreground mb-1">
            ...
            <span className={`font-medium ${activeSeq.status === "pending_approval" ? "text-amber-400" : "text-blue-400"}`}>
              {activeSeq.status === "pending_approval" ? "⏳ Awaiting Approval" : "Active"}
            </span>
          </div>
      ...
      <div className="flex gap-1.5 mt-1">
          ...
          <button
            onClick={(e) => { 
              ...
              if (confirm(`Move ${university.university_name} back a step?`)) {
                setIsReverting(true);
                revertStage({ id: university._id }).finally(() => setIsReverting(false));
              }
            }}
        ...
        {(university.outreach_stage === "outreach_active" || university.outreach_stage === "replied") && (
          <button
            onClick={(e) => { e.stopPropagation(); onSimulateReply(); }}
...
function UniversityTimelineDrawer({
  ...
  const emails = useQuery(api.emails.listByUniversity, { university_id: universityId });
  const replies = useQuery(api.replies.listByUniversity, { university_id: universityId });
  const sequences = useQuery(api.sequences.listByUniversity, { university_id: universityId });
  ...
  const timeline: TimelineEvent[] = [
    ...(emails ?? []).map((e) => ({ ts: e.sent_at ?? e._creationTime, type: "email", data: e })),
    ...(replies ?? []).map((r) => ({ ts: r.received_at, type: "reply", data: r })),
  ].sort((a, b) => b.ts - a.ts);

  const activeSeq = sequences?.find(s => s.status === "active" || s.status === "pending_approval" || s.status === "completed");
  ...
  return (
    <div className="fixed right-0 top-0 h-full w-[480px] bg-card border-l border-card-border z-50 flex flex-col shadow-2xl">
        ...
        <div>
          ...
          {activeSeq && (
            <p className="text-xs text-muted-foreground mt-0.5">
              ...
              <span className={`font-semibold ${activeSeq.status === "completed" ? "text-emerald-400" : activeSeq.status === "pending_approval" ? "text-amber-400" : "text-blue-400"}`}>
      ...
      {uni && (
        <div className="px-5 py-3 bg-background border-b border-card-border/40 flex items-center gap-2">
          ...
          <span className={`text-xs font-bold uppercase px-2 py-0.5 rounded-full ${
            uni.outreach_stage === "meeting_booked" ? "bg-amber-500/10 text-amber-400" :
            uni.outreach_stage === "replied" ? "bg-emerald-500/10 text-emerald-400" :
            uni.outreach_stage === "outreach_active" ? "bg-blue-500/10 text-blue-400" :
            "bg-muted text-muted-foreground"
          }`}>
            {(uni.outreach_stage ?? "new").replace(/_/g, " ")}
          </span>
          <span className="text-xs text-muted-foreground ml-auto">{timeline.length} events</span>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-3 custom-scrollbar">
        {timeline.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground text-sm">No activity yet for this university.</div>
        ) : (
                  ...
                  return (
                    <div key={i} className="flex gap-3 relative">
                      <div className="w-7 h-7 rounded-full bg-blue-500/10 border border-blue-500/20 flex items-center justify-center flex-shrink-0 z-10">
                        {EMAIL_STATUS_ICON[e.status] ?? <span className="w-2 h-2 rounded-full bg-zinc-500 inline-block" />}
                      </div>
                      <div className="flex-1 bg-muted border border-card-border/60 rounded-xl p-3 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs font-semibold text-foreground">
                            {e.step_number === 99 ? "Auto-Reply" : `Step ${e.step_number}`} Email
                          </span>
                          <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-full border ${
                            e.status === "opened" || e.status === "clicked" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20" :
                            e.status === "sent" || e.status === "delivered" ? "bg-blue-500/10 text-blue-400 border-blue-500/20" :
                            e.status === "pending_approval" ? "bg-amber-500/10 text-amber-400 border-amber-500/20" :
                            e.status === "bounced" ? "bg-red-500/10 text-red-400 border-red-500/20" :
                            "bg-muted text-muted-foreground border-card-border"
                          }`}>
                            {e.status}
                          </span>
                        </div>
                        <p className="text-xs text-foreground font-medium truncate">{e.subject}</p>
                        {e.stakeholder_name && (
                          <p className="text-[10px] text-muted-foreground mt-0.5">To: {e.stakeholder_name} {e.stakeholder_email ? `<${e.stakeholder_email}>` : ""}</p>
                        )}
                        <p className="text-[10px] text-zinc-600 mt-1.5">
                          {new Date(event.ts).toLocaleString([], { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                        </p>
                      </div>
                    </div>
                  );
                  ...
                  return (
                    <div key={i} className="flex gap-3 relative">
                      ...
                      <div className="flex-1 bg-emerald-500/5 border border-emerald-500/20 rounded-xl p-3 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs font-semibold text-emerald-400">Inbound Reply</span>
                          {r.classification && (
                            <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-full ${CLASSIFICATION_STYLES[r.classification] ?? CLASSIFICATION_STYLES.other}`}>
                              {r.classification.replace(/_/g, " ")}
                            </span>
                          )}
                        </div>
                        <p className="text-xs text-foreground line-clamp-3 leading-relaxed">{r.raw_reply}</p>
                        <p className="text-[10px] text-zinc-600 mt-1.5">
...
function SimulateReplyModal({
  ...
  const stakeholders = useQuery(api.stakeholders.listByUniversity, { university_id: university._id });
  ...
  const primarySt = stakeholders?.find(s => s.is_primary) ?? stakeholders?.[0];
  ...
  const PRESETS = [
    ...
    { label: "✅ Positive Interest", text: "This looks interesting. Could you send me more details about the platform pricing and features?" },
    { label: "ℹ️ Info Request", text: "What modules do you offer specifically for hostel management? We're looking to digitize our hostel operations." },
    { label: "❌ Not Interested", text: "Thank you for reaching out, but we're not looking for new software solutions at the moment." },
    { label: "🏖️ Out of Office", text: "Thank you for your email. I am currently out of the office until March 20th and will respond upon my return." },
  ];
  ...
  const handleSimulate = async () => {
    ...
    try {
      const replyId = await createReply({
        ...
        stakeholder_id: selectedStakeholderId as Id<"stakeholders">,
        raw_reply: replyText,
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="bg-card border border-card-border/80 rounded-2xl p-6 w-full max-w-lg shadow-2xl">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-lg font-bold font-heading text-foreground">Simulate Inbound Reply</h2>
            <p className="text-muted-foreground text-sm">{university.university_name}</p>
          </div>
        ...
        {done ? (
          <div className="text-center py-8">
            <div className="text-4xl mb-3">
              {done === "meeting_request" ? "🎉" : done === "positive_interest" ? "✅" : done === "not_interested" ? "❌" : "📧"}
            </div>
            <p className="text-foreground font-semibold mb-1">Reply Processed!</p>
            <p className="text-muted-foreground text-sm mb-1">Classification:</p>
            <span className={`text-sm font-bold uppercase px-3 py-1 rounded-full ${CLASSIFICATION_STYLES[done] ?? CLASSIFICATION_STYLES.other}`}>
              {done.replace(/_/g, " ")}
            </span>
            {done === "meeting_request" && (
              <p className="text-emerald-400 text-xs mt-3 font-medium">🚀 Auto-proposal generation triggered!</p>
            )}
            <p className="text-muted-foreground text-xs mt-3">
              Stage updated and auto-reply sent (if applicable). Check the Kanban board and Replies panel.
            </p>
            <button onClick={onClose} className="mt-5 px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold rounded-lg transition-colors">
              Done
            </button>
          </div>
        ) : (
          <>
            {/* Stakeholder */}
            <div className="mb-4">
              <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Replying Stakeholder</label>
              <select
            ...
            {/* Presets */}
            <div className="mb-3">
              <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Quick Presets</label>
            ...
            {/* Reply text */}
            <div className="mb-5">
              <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Reply Content</label>
              <textarea
                value={replyText}
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={!replyText.trim() || !selectedStakeholderId || running}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-card border border-card-border/60 shadow-2xl rounded-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="p-4 border-b border-card-border/60 flex items-center gap-3 bg-card/40">
          ...
          <span className="text-foreground font-semibold text-base flex-1">Skipped Universities</span>
        ...
        <div className="flex border-b border-card-border/60 bg-card/20 px-4">
          {[
            ...
            { id: "list" as const, label: `Skipped List${skippedList ? ` (${skippedList.length})` : ""}` },
          ].map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
        ...
        {tab === "skip" && (
          <>
            <div className="p-4 border-b border-card-border/40 flex items-center gap-2 bg-background/30">
              ...
              />
              {searchTerm && (
                <button onClick={() => setSearchTerm("")} className="p-1 hover:bg-muted rounded-lg text-muted-foreground">
            ...
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
              {!searchTerm ? (
                <div className="py-12 text-center text-muted-foreground text-sm">
                  Type to search. Skipped universities will no longer be eligible for outreach.
                </div>
              ) : results === undefined ? (
                ...
                <div className="py-12 text-center text-muted-foreground text-sm">No universities found</div>
              ) : (
                <div className="space-y-2">
                  {results.map((uni) => (
                    <div key={uni._id} className="flex items-center justify-between p-4 bg-muted rounded-xl border border-card-border gap-4">
                      <div className="min-w-0">
                        <h4 className="text-foreground font-medium mb-0.5 truncate">{uni.university_name}</h4>
          ...
          <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
            {skippedList === undefined ? (
              ...
              <div className="py-16 text-center">
                ...
                <p className="text-foreground font-semibold mb-1">No skipped universities</p>
                <p className="text-muted-foreground text-sm">Universities you skip will appear here for review.</p>
              </div>
            ) : (
              <div className="space-y-2">
                <p className="text-xs text-muted-foreground mb-3 px-1">
                  {skippedList.length} {skippedList.length === 1 ? "university" : "universities"} currently skipped.
                  Unskipping moves them back to <span className="text-sky-400 font-medium">Ready to Sequence</span>.
                </p>
                {skippedList.map((uni) => (
                  <div key={uni._id} className="flex items-center justify-between p-4 bg-muted rounded-xl border border-card-border gap-4">
                    <div className="min-w-0">
