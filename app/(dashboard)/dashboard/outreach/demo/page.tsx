...
export default function DemoPage() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      autoPlayIdxRef.current =
        (autoPlayIdxRef.current + 1) % PIPELINE_COLUMNS.length;
      setHighlightStage(PIPELINE_COLUMNS[autoPlayIdxRef.current]);
    }, 2500);
  ...
  const totalReplied =
    ...
    (grouped["meeting_booked"] ?? []).length;
  const replyRate =
    totalActive > 0 ? Math.round((totalReplied / totalActive) * 100) : 0;

  const KPI_CARDS = [
    {
      label: "Universities in Pipeline",
    ...
    },
    {
      ...
      value: funnel?.outreachActive ?? 0,
      color: "text-sky-400",
      icon: "✉️",
    },
    {
      label: "Meetings Booked",
      value: funnel?.meetingBooked ?? 0,
      color: "text-amber-400",
      icon: "📅",
    },
    {
  ...
  return (
    <div
      className={`min-h-screen bg-background transition-all duration-500 ${isPresenting ? "p-6" : "p-8 pb-20"}`}
    >
      {/* ── Header ── */}
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={() => {
              ...
              if (!autoPlay) setHighlightStage(PIPELINE_COLUMNS[0]);
              else setHighlightStage(null);
            }}
      ...
      <div className="grid grid-cols-6 gap-4 mb-8">
        {KPI_CARDS.map((kpi) => (
          <div
            key={kpi.label}
            className="bg-card border border-card-border/60 rounded-2xl p-5 shadow-sm hover:border-zinc-600 transition-colors"
          >
            <div className="text-2xl mb-2">{kpi.icon}</div>
            <p className={`text-3xl font-bold font-heading ${kpi.color}`}>
              {kpi.value}
            </p>
            <p className="text-muted-foreground text-xs font-medium mt-1 uppercase tracking-wide">
              {kpi.label}
            </p>
          </div>
        ))}
        ...
        <div className="flex gap-2 overflow-x-auto pb-2">
          {PIPELINE_COLUMNS.map((stage) => {
            ...
            const pct = funnel?.total
              ? Math.round((count / funnel.total) * 100)
              : 0;
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span
                    className={`text-[10px] font-bold uppercase tracking-widest ${isHighlighted ? cfg.color : "text-muted-foreground"}`}
                  >
                    {cfg.label}
                  </span>
                </div>
                <p
                  className={`text-4xl font-bold font-heading mb-1 ${isHighlighted ? cfg.color : "text-foreground"}`}
                >
                  {count}
                </p>
      ...
      {highlightStage && (grouped[highlightStage] ?? []).length > 0 && (
        <div className="mb-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
          <div
            className={`rounded-2xl border p-6 ${STAGE_CONFIG[highlightStage]?.bg}`}
          >
            <div className="flex items-center gap-2 mb-4">
              <span
                className={`text-sm font-bold uppercase tracking-widest ${STAGE_CONFIG[highlightStage]?.color}`}
              >
                {STAGE_CONFIG[highlightStage]?.label} — Universities
              </span>
              <span
                className={`text-xs font-bold px-2 py-0.5 rounded-full ${STAGE_CONFIG[highlightStage]?.bg} ${STAGE_CONFIG[highlightStage]?.color} border`}
              >
                {grouped[highlightStage]?.length}
              </span>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              {(grouped[highlightStage] ?? [])
                .slice(0, 8)
                .map((uni: Doc<"universities">) => (
                  <div
                    key={uni._id}
                    className="bg-background/60 rounded-xl p-3.5 border border-card-border/60"
                  >
                    <p className="text-foreground font-semibold text-sm truncate mb-0.5">
                      {uni.university_name}
                    </p>
                    ...
                    {uni.lead_tier && (
                      <span
                        className={`text-[9px] font-bold uppercase mt-1.5 inline-block ${
                          uni.lead_tier === "High"
                            ? "text-emerald-400"
          ...
          <div className="bg-card border border-card-border/60 rounded-2xl overflow-hidden max-h-64 overflow-y-auto shadow-sm">
            {!replies || replies.length === 0 ? (
              ...
              <div className="divide-y divide-zinc-800/60">
                {replies.slice(0, 6).map((reply: Doc<"replyLogs">) => (
                  <div
                    key={reply._id}
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 mb-1">
                      ...
                      {reply.classification && (
                        <span
                          className={`text-[9px] font-bold uppercase flex-shrink-0 px-1.5 py-0.5 rounded-full border ${
                            reply.classification === "meeting_request"
                              ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
                              : reply.classification === "positive_interest"
          ...
          <div className="bg-card border border-card-border/60 rounded-2xl p-5 shadow-sm space-y-4">
            {[
              ...
              },
              {
                ...
                pct: funnel?.total
                  ? Math.round((funnel.enriched / funnel.total) * 100)
                  : 0,
                color: "bg-sky-500",
              },
              {
                from: "Active Outreach",
                value: funnel?.outreachActive ?? 0,
                pct: funnel?.total
                  ? Math.round((funnel.outreachActive / funnel.total) * 100)
                  : 0,
                color: "bg-indigo-500",
              },
              {
                ...
                pct: funnel?.total
                  ? Math.round((funnel.replied / funnel.total) * 100)
                  : 0,
              ...
              },
              {
                ...
                value: funnel?.meetingBooked ?? 0,
                pct: funnel?.total
                  ? Math.round((funnel.meetingBooked / funnel.total) * 100)
                  : 0,
                color: "bg-amber-500",
              },
              {
      ...
      <div className="mt-10 pt-6 border-t border-card-border/40 flex items-center justify-between">
        <div>
          <p className="text-xs text-muted-foreground">
            Powered by{" "}
            <span className="font-bold text-foreground">Fretbox</span> Outreach
            AI
          </p>
          <p className="text-[11px] text-zinc-600 mt-0.5">
            Real-time data · Zero lag · Auto-classification · AI proposals
          </p>
        </div>
