...
export default function ProposalsPage() {
  ...
  const allStakeholders = useQuery(api.stakeholders.listByUniversity, 
      selectedUniId ? { university_id: selectedUniId as Id<"universities"> } : "skip"
  );

  const filteredStakeholders = useMemo(() => {
    return allStakeholders?.filter(s => s.email && s.email.trim() !== "" && s.email.trim().toLowerCase() !== "null") ?? [];
  }, [allStakeholders]);

  const filteredUnis = useMemo(() => {
    if (!allUnis) return [];
    if (!searchQuery) return allUnis;
    return allUnis.filter(u => u.university_name.toLowerCase().includes(searchQuery.toLowerCase()));
  }, [allUnis, searchQuery]);
  ...
  const handleManualGenerate = async () => {
    ...
    try {
      const proposalId = await createProposal({
        ...
        stakeholder_id: selectedStakeholderId ? (selectedStakeholderId as Id<"stakeholders">) : undefined,
        meeting_date: Date.now(),
      });
      await generateProposal({
        universityId: selectedUniId as Id<"universities">,
        proposalId,
        stakeholderId: selectedStakeholderId ? (selectedStakeholderId as Id<"stakeholders">) : undefined,
      });
      ...
      <div className="mb-8 flex items-start justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1.5 font-medium">
            AI-generated deal proposals — auto-created when a meeting is booked, or manually here
          </p>
        </div>
        <button
      ...
      {showManualModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-card-border/80 rounded-2xl p-6 w-full max-w-lg shadow-2xl flex flex-col max-h-[90vh]">
            <h2 className="text-xl font-bold font-heading text-foreground mb-1">Generate Proposal</h2>
            <p className="text-muted-foreground text-sm mb-6">Select a university and an optional stakeholder to address.</p>
            
            <div className="flex-1 overflow-y-auto pr-2 space-y-5">
              {/* Step 1: Uni Selection */}
              <div>
                <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">1. Select University</label>
                <input
                  type="text"
                ...
                >
                  ...
                  {filteredUnis?.length === 0 && <option disabled>No universities found...</option>}
                </select>
              </div>
              ...
              {selectedUniId && (
                <div className="animate-in fade-in slide-in-from-top-4 duration-300">
                  <label className="block text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">2. Address To (Optional)</label>
                  {!allStakeholders ? (
                    <div className="text-sm text-muted-foreground animate-pulse py-2">Loading contacts...</div>
                  ) : filteredStakeholders.length === 0 ? (
                    <div className="text-sm text-yellow-500 bg-yellow-500/10 px-3 py-2 rounded-lg border border-yellow-500/20">
                      No stakeholders with email found for this university. Proposal will be generically addressed.
                    </div>
                  ) : (
                    <select
...
function ProposalCard({ proposal }: { proposal: Doc<"proposals"> & { university_name?: string, stakeholder_details?: string } }) {
  ...
  const allStakeholders = useQuery(api.stakeholders.listByUniversity, { university_id: proposal.university_id });
  const validStakeholders = useMemo(
    () => allStakeholders?.filter((s) => s.email && s.email.trim() && s.email.toLowerCase() !== "null") ?? [],
    [allStakeholders]
  );
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try { await removeProposal({ id: proposal._id }); }
    catch (e) { alert(`Delete failed: ${e}`); setDeleting(false); }
  };
  ...
  const handleRegenerate = async () => {
    ...
    try {
      await generateProposal({
        ...
        ...(proposal.stakeholder_id ? { stakeholderId: proposal.stakeholder_id } : {}),
      });
        ...
        <div className="p-6 flex flex-col h-full">
          ...
          <div className="flex items-start justify-between mb-4">
            ...
            <span className={`px-2 py-1 rounded-md text-[10px] uppercase font-bold tracking-wider border ${
              isSent  ? "bg-blue-500/10 text-blue-400 border-blue-500/20"     :
          ...
          <div className="flex items-center text-muted-foreground text-xs mb-4">
            ...
            {proposal.meeting_date ? new Date(proposal.meeting_date).toLocaleDateString() : "TBD"}
          </div>

          <div className="mb-4">
            <span className="inline-block text-[10px] uppercase tracking-wider font-bold text-muted-foreground/70 mb-1">Prepared For</span>
          ...
          {summaryText && (
            <div className="mb-4">
              ...
              <p className="text-foreground text-xs leading-relaxed line-clamp-3">{summaryText}</p>
            </div>
          )}

          {content?.key_benefits && Array.isArray(content.key_benefits) && content.key_benefits.length > 0 && (
            <div className="mb-4">
              <p className="text-muted-foreground text-[10px] uppercase font-bold tracking-widest mb-1.5">Key Benefits</p>
              <div className="space-y-1">
                {content.key_benefits.slice(0, 2).map((b: string, i: number) => (
                  <p key={i} className="text-foreground text-xs leading-relaxed line-clamp-1">• {b}</p>
                ))}
                {content.key_benefits.length > 2 && (
                  <p className="text-muted-foreground text-[10px]">+{content.key_benefits.length - 2} more</p>
                )}
              </div>
            </div>
          )}
          ...
          <div className="flex flex-col gap-2 mt-auto pt-2">
            {confirmDelete ? (
              <div className="bg-red-950/30 border border-red-500/30 rounded-xl p-3 flex flex-col gap-2">
                <p className="text-xs text-red-400 font-medium">Delete this proposal permanently?</p>
                <div className="flex gap-2">
                  <button onClick={() => setConfirmDelete(false)} className="flex-1 py-1.5 text-xs font-medium bg-muted text-muted-foreground rounded-lg hover:bg-zinc-700 transition-colors">Cancel</button>
                  <button onClick={handleDelete} disabled={deleting} className="flex-1 py-1.5 text-xs font-bold bg-red-600 hover:bg-red-500 disabled:opacity-50 text-white rounded-lg transition-colors flex items-center justify-center gap-1.5">
                    {deleting ? <ArrowPathIcon className="w-3 h-3 animate-spin" /> : <TrashIcon className="w-3 h-3" />}
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setConfirmDelete(true)} className="p-2 rounded-xl bg-muted border border-card-border/50 text-muted-foreground hover:text-red-400 hover:border-red-500/30 transition-colors" title="Delete proposal">
                  <TrashIcon className="w-4 h-4" />
                </button>
                {isDraft ? (
                  <button disabled className="flex-1 bg-muted border border-card-border/50 text-foreground/50 py-2 rounded-xl text-sm font-semibold cursor-not-allowed flex items-center justify-center gap-2">
                    <ClockIcon className="w-4 h-4 animate-pulse" /> Generating...
                  </button>
                ) : !content ? (
                  <button onClick={withKeyCheck(handleRegenerate)} disabled={regenerating} className="flex-1 bg-amber-600/20 border border-amber-500/30 hover:bg-amber-600/30 text-amber-400 py-2 rounded-xl text-sm font-semibold transition-colors flex items-center justify-center gap-2">
                    <ArrowPathIcon className={`w-4 h-4 ${regenerating ? "animate-spin" : ""}`} />
                    {regenerating ? "Regenerating..." : "Regenerate Content"}
                  </button>
                ) : (
                  <button onClick={() => setShowEditModal(true)} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white py-2 rounded-xl text-sm font-semibold transition-colors flex items-center justify-center gap-2 shadow-sm">
...
────────────────────────────────────────────────────────── */
function AutoResizeTextareaField({ label, value, onChange, hint }: { label: string; value: string; onChange: (v: string) => void; hint?: string }) {
  ...
  return (
    <div className="group relative -mx-4 px-4 py-2 hover:bg-muted/30 rounded-2xl transition-colors">
      <div className="flex items-center justify-between pl-1 mb-1.5 transition-opacity opacity-40 group-focus-within:opacity-100 group-hover:opacity-100">
        <label className="text-[10px] font-bold uppercase tracking-widest text-blue-500">{label}</label>
        {hint && <span className="text-[10px] text-muted-foreground/60 invisible sm:visible">{hint}</span>}
      </div>
      <textarea
        ref={textareaRef}
        value={value}
...
function ProposalEditModal({
  ...
  const [whyNow,   setWhyNow]   = useState<string>(initialContent?.executive_summary?.why_now || "");
  const [vision,   setVision]   = useState<string>(initialContent?.executive_summary?.vision_statement || "");
  const [problem,  setProblem]  = useState<string>(initialContent?.problem_statement || "");
  const [solution, setSolution] = useState<string>(initialContent?.solution_overview || "");
  const [benefits, setBenefits] = useState<string>(
    Array.isArray(initialContent?.key_benefits) ? initialContent.key_benefits.join("\n") : (initialContent?.key_benefits || "")
  );
  const [roi,       setRoi]      = useState<string>(initialContent?.roi_summary || "");
  const [nextSteps, setNextSteps]= useState<string>(
    Array.isArray(initialContent?.next_steps) ? initialContent.next_steps.join("\n") : (initialContent?.next_steps || "")
  );
  ...
  const handleSend = async () => {
    if (selectedEmails.length === 0 && !ccEmailsStr.trim()) { alert("Select at least one recipient or provide a CC."); return; }
    setSending(true);
    try {
      ...
      <div className="relative w-full max-w-6xl h-full max-h-[90vh] bg-background border border-card-border/80 rounded-2xl flex flex-col md:flex-row shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-8 lg:p-10 hide-scrollbar z-0">
            ...
            <div className="max-w-3xl mx-auto bg-card border border-card-border/80 shadow-sm rounded-2xl p-6 sm:p-10 lg:p-14 space-y-2">
              ...
              <div className="mb-8 border-b border-card-border/50 pb-6">
                <h2 className="text-3xl font-heading font-bold text-foreground mb-2">{proposal.university_name}</h2>
                <p className="text-sm font-medium text-muted-foreground uppercase tracking-wider">Proposal Draft</p>
              </div>

              <AutoResizeTextareaField label="Executive Hook" value={hook} onChange={setHook} hint="Opening line to create immediate curiosity." />
              <AutoResizeTextareaField label="Why Now" value={whyNow} onChange={setWhyNow} hint="Urgency & timing for acting now." />
              <AutoResizeTextareaField label="Vision Statement" value={vision} onChange={setVision} hint="The future state you're helping them achieve." />
              <AutoResizeTextareaField label="The Challenge" value={problem} onChange={setProblem} hint="The problem they're currently facing." />
              <AutoResizeTextareaField label="Our Solution" value={solution} onChange={setSolution} hint="How Fretbox specifically solves their challenge." />
              <AutoResizeTextareaField label="Key Benefits" value={benefits} onChange={setBenefits} hint="One benefit per line." />
              <AutoResizeTextareaField label="Expected ROI" value={roi} onChange={setRoi} hint="Quantified or qualified outcome." />
              <AutoResizeTextareaField label="Next Steps" value={nextSteps} onChange={setNextSteps} hint="One step per line." />
            </div>
          </div>
        </div>
        ...
        <div className="w-full md:w-80 lg:w-96 bg-card border-t md:border-t-0 md:border-l border-card-border/80 flex flex-col flex-shrink-0 z-20 shadow-xl">
          ...
          <div className="px-6 py-5 border-b border-card-border/60 flex items-center justify-between">
            <h3 className="font-semibold text-foreground tracking-tight">Send Proposal</h3>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8">
            ...
            <div>
              ...
              >
                ...
                {saved ? "✓ Saved to Drafts" : saving ? "Saving..." : "Save Draft"}
              </button>
            </div>

            {/* Recipients */}
            <div>
              <h4 className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-3">Recipients</h4>
              ...
              {validStakeholders.length === 0 ? (
                <p className="text-xs text-yellow-500 bg-yellow-500/10 px-3 py-2 rounded-lg border border-yellow-500/20">
                  No stakeholders with email addresses found. Provide CC emails below.
                </p>
              ) : (
                  ...
                  {validStakeholders.map((st) => (
                    <label key={st._id} className="flex items-start gap-3 p-3 rounded-xl bg-background border border-card-border/50 cursor-pointer hover:border-emerald-500/30 transition-colors">
                      <input
                        ...
                        onChange={e => {
                          if (e.target.checked) setSelectedEmails(prev => [...prev, st.email!]);
                          else setSelectedEmails(prev => prev.filter(em => em !== st.email!));
                        }}
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-muted-foreground truncate mt-0.5">{st.role} • {st.email}</p>
                      </div>
                    </label>
                  ))}
                </div>
            ...
            {/* CC */}
            <div>
              <h4 className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground mb-2">CC (Optional)</h4>
              <textarea
                rows={2}
          ...
          <div className="p-6 border-t border-card-border/60 bg-card">
            <button
              onClick={handleSend}
              disabled={sending || (selectedEmails.length === 0 && !ccEmailsStr.trim())}
